...
if ~isempty(obs_mode)
    ...
    if any(obs_mode==2)        
        ...
        len = distdim(1000,'ft','nm');
        breadth = distdim(500,'ft','nm');
        height_cub = 0.55;
        height_res_cub = 0.03;
...
%     psii_seq = chii_seq;
    ...
    trans = other_states(:,1:3,:) - states(:,1:3,:); % assumed to determined by sensor measurements (after transformation from body to inertial)
    L_b = mtimesx(permute(C_own_NED,[2 1 3]),permute(trans,[2,1,3]));  % Positions in Body Frame cartesian (actually measured by sensors)
...
%% Plot System
path = true;
fig1 = plot_system_seq(Yhist,tar0,collision_over_time_seq,obs,path);
hold on
% envelope_draw;
return
...
if animate, fig3 = animate_system_seq(Yhist,Thist,tar0,obs,collision_over_time_seq,VIDEO,video_title,t_inc); end
hold on
