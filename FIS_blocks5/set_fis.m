...
for i = 1:length(rule)
    switch rule(i).FISName
        case {"FIS_A2", "FIS_A3", "FIS_B2", "FIS_B3"}
...
% in = [d_h, del_psi_oi, del_chi_d, V_x, d_h, del_chi_ca, del_V_ca_h, 
% d_v_mag, d_v, del_h_d, V_z, d_v, del_h_ca, del_V_ca_z] 
...
fis = setTunableValues(fis,in(3),four_param(vec(5:6),angle_pi)); % del_chi_d
fis = setTunableValues(fis,in(4),four_param(vec(7:8),vel_kts_mag)); % V_x
fis = setTunableValues(fis,in(5),four_param(vec(1:2),dist_nm_mag)); % d_h
fis = setTunableValues(fis,in(6),four_param(vec(9:10),del_angle_pi)); % del_chi_ca
fis = setTunableValues(fis,in(7),four_param(vec(11:12),del_vel_kts)); % del_V_ca_h
fis = setTunableValues(fis,in(8),four_param(vec(13:14),dist_ft_mag)); % d_v_mag
fis = setTunableValues(fis,in(9),four_param(vec(15:16),dist_ft)); % d_v
fis = setTunableValues(fis,in(10),four_param(vec(17:18),dist_ft)); % del_h_d
fis = setTunableValues(fis,in(11),four_param(vec(19:20),vel_ftm)); % V_z
fis = setTunableValues(fis,in(12),four_param(vec(15:16),dist_ft)); % d_v
...
fis = setTunableValues(fis,out(1),four_param(vec(9:10),del_angle_pi)); % del_chi_ca
fis = setTunableValues(fis,out(2),four_param(vec(11:12),del_vel_kts)); % del_V_ca_h
fis = setTunableValues(fis,out(3),four_param(vec(25:26),unit_mag)); % W_h
fis = setTunableValues(fis,out(4),four_param(vec(21:22),del_dist_ft)); % del_h_ca
fis = setTunableValues(fis,out(5),four_param(vec(23:24),del_vel_ftm)); % del_V_ca_z
fis = setTunableValues(fis,out(6),four_param(vec(27:28),unit_mag)); % W_v
