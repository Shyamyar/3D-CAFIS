...
chi_d = wrapTo2Pi(atan2(trans_t(:, 2), trans_t(:, 1)));    % desired course angle towards target
gamma_d = atan2(-trans_t(:, 3), d_h_t);         % desired pitch angle towards target

k_v = th.Va_opt / (5 * th.d_near^2);            % Time to reach destination with optimal vel from initial position
...
Va_d(~check1) = th.Va_opt - k_v .* (th.d_near - d_slant_t(~check1)).^2; % using tmax as reference for V_opt
...
del_z_d = convlength(del_pos_lookahead(:, 3),'naut mi','ft');   % in ft in w.r.t current position
chi = states(:, 7); % current course angle
del_chi_d = wrapToPi(chi_d - chi); % change in course angle required

des_states = [Va_d, chi_d, h_d, del_chi_d, del_z_d];